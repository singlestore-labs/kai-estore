...
function encrypt(value) {
  ...
  return crypto.AES.encrypt(_value, CRYPTO_SECRET2).toString();
}
function decrypt(value) {
  return crypto.AES.decrypt(value, CRYPTO_SECRET2).toString(crypto.enc.Utf8);
}
...
var connectionRouter = express12.Router();
var { DB_URI: DB_URI2, DB_NAME: DB_NAME2, DATA_SIZE: DATA_SIZE2, IS_SINGLE_DB: IS_SINGLE_DB2 } = processEnv();
var validationSchema = IS_SINGLE_DB2 ? void 0 : zod4.object({
  body: zod4.object({
    mongoURI: zod4.string({ required_error: "MongoURI is required" }).nonempty(),
    dbName: zod4.string({ required_error: "Database name is required" }).nonempty(),
    dataSize: zod4.string({ required_error: "Dataset size is required" }).nonempty()
  })
});
...
// src/routes/data.ts
var dataRouter = express12.Router();
...
dataRouter.post(
  "/data/set",
  validateRoute(zod4.object({ query: zod4.object({ force: zod4.string().optional() }) })),
  async (req, res, next) => {
    try {
...
// src/routes/user.ts
var userRouter = express12.Router();
...
var categoryRouter = express12.Router();
...
var tagRouter = express12.Router();
...
var infoRouter = express12.Router();
...
var ratingRouter = express12.Router();
ratingRouter.post(
  "/rating",
  validateRoute(
    zod4.object({
      body: zod4.object({
        productId: zod4.string({ required_error: "Product id is required" }).nonempty(),
        value: zod4.number({ required_error: "Value is required" }).min(0).max(5)
      })
    })
  ),
  async (req, res, next) => {
...
var productRouter = express12.Router();
var validateId = zod4.object({ params: zod4.object({ id: zod4.string().nonempty().optional() }).optional() });
var validateNumber = zod4.object({ query: zod4.object({ number: zod4.string().optional() }) });
var validateFrom = zod4.object({
  query: zod4.object({ from: zod4.string().nonempty().optional() }).optional()
});
...
productRouter.get(`/product/:id/sales`, validateRoute(validateId, validateFrom), async (req, res) => {
  try {
    ...
    const from = query.from ? new Date(query.from) : subDays(/* @__PURE__ */ new Date(), 90);
...
var orderRouter = express12.Router();
orderRouter.post(
  "/order",
  validateRoute(zod4.object({ body: zod4.object({ productIds: zod4.array(zod4.string()) }) })),
  async (req, res, next) => {
    try {
      const {
        db,
...
var configRouter = express12.Router();
...
// src/routes/index.ts
var withConnectionRouter = express12.Router();
...
var apiRouter = express12.Router();
...
function initSocket(server2, onInit, onConnection) {
  socketServer = new Server(server2, { cors: { origin: ORIGINS2, credentials: true }, cookie: true });
...
app.use(morgan("dev"));
app.use(cors({ origin: ORIGINS3, credentials: true }));
app.use(helmet());
app.use(cookieParser());
app.use(express12.json());
app.use("/images", express12.static(path.join(getDirname(import.meta.url), "data/images")));
