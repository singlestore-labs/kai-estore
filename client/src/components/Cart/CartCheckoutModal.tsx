...
export function CartCheckoutModal({
  ...
  if (cartProducts.length) {
    cartBody = (
      <ProductList
        ...
        cardChildren={(productId) => (
          <Button variant="ghost" flex="0 0 auto" onClick={() => handleRemoveClick(productId)}>
            <Icon name="solid.faTrash" w="2.5" />
          </Button>
        )}
        withDivider
        mt="6"
    ...
    cartBody = (
      <Box mt="6">
        <Typography as="p" fontSize="md" lineHeight="6" fontWeight="semibold">
          There are no items in your cart.
        </Typography>
        <Button variant="solid" size="s2.md" mt="3" onClick={onContinueShopingClick}>
          Continue Shopping
        </Button>
      </Box>
    );
  ...
  return (
    <Modal {...props} isOpen={isOpen} size="s2.md">
      <Flex alignItems="flex-start" justifyContent="flex-start" gap="9">
        {cartTitle}
        {summary}
      </Flex>
      {productsSectionAlso}
