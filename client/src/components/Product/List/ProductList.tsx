...
export function ProductList({
    ...
    return (
      ...
      >
        {Array.from({ length: skeletonWrapperProps?.number ?? 1 }).map((_, i) => (
          <Skeleton key={i} ratio={100} {...skeletonProps} />
        ))}
      </SkeletonWrapper>
    );
  }
  ...
  return (
    <Box position="relative" overflowX="hidden" overflowY="auto" {...variantProps.list} {...props}>
      {_products.map((product) => (
        <Fragment key={product.id}>
          <ProductCard {...product} variant={variant} {...variantProps.productCard}>
            {cardChildren}
          </ProductCard>
          {divider}
        </Fragment>
