...
export function DbSourceCard({
  ...
  if (title || logo) {
    let _logo;
    if (logo) {
      _logo = <Logo variant={logo} color="currentcolor" mr="2" />;
    }

    _title = (
      <Typography as="span" display="inline-flex" alignItems="center" justifyContent="flex-start">
        {_logo}
        {title}
      </Typography>
    );
  ...
  if (isActive && (subtitle || subtitlePrefix)) {
    ...
    if (subtitlePrefix) {
      _prefix = (
        <Typography as="span" fontSize="2xl" textDecoration="underline">
          {subtitlePrefix}
        </Typography>
      );
    }
    ...
    _subtitle = (
      <Typography as="span" display="flex" alignItems="baseline" gap="1">
        {_prefix}
        {_text}
      </Typography>
    );
